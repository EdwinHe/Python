...
def refresh_mapping(db_h):
    for mapping in configs.keyword_mappings:
        ...
        if dbUtils.is_new_mapping(db_h, mapping[0]):
            configs.mappings_imported += 1
...
def display_status():
    print("Mappings imported:", configs.mappings_imported)
    print("Files imported:", configs.files_imported)
    print("Records imported:", configs.records_imported)
